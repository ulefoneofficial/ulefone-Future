...
//	int delay;
	...
	switch (cmd) {
		...
		case MSENSOR_IOCTL_READ_SENSORDATA:	
			st480_work_func();
			printk(KERN_ERR "[st480mc] read msensor data: %d, %d, %d\n", mag.mag_x, mag.mag_y, mag.mag_z);
		...
		case MSENSOR_IOCTL_SET_POSTURE:
			printk("st480 MSENSOR_IOCTL_SET_POSTURE\n");
			if(argp == NULL)
			{
				MSE_ERR("IO parameter pointer is NULL!\r\n");
				break;
			...
			printk("st480 yaw = %d, pitch = %d, roll = %d\n", st480sensordata.yaw, st480sensordata.pitch, st480sensordata.roll);
			break;

		case MSENSOR_IOCTL_SET_CALIDATA:
			printk("st480 MSENSOR_IOCTL_SET_CALIDATA\n");
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			printk(KERN_ERR "%s sensor_get_data\n",__FUNCTION__);
			...
			else
			{
				...
				printk(KERN_ERR "%s, SENSOR_GET_DATA, data:%d,%d,%d\n",__FUNCTION__,msensor_data->values[0],msensor_data->values[1],msensor_data->values[2]);
			}
			break;
		default:
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				printk(KERN_ERR "%s, SENSOR_GET_DATA, data:%d,%d,%d\n",__FUNCTION__,osensor_data->values[0],osensor_data->values[1],osensor_data->values[2]);				
			}
			break;
		default:
...
static int st480_o_get_data(int* x ,int* y,int* z, int* status)
{
	...
	*status = st480sensordata.mag_status;
       printk(KERN_ERR "%s, data:%d,%d,%d\n", __FUNCTION__,st480sensordata.yaw, st480sensordata.pitch, st480sensordata.roll);
	return 0;
}
#endif
...
static int __init st480_init(void)
{
	...
	if (mag_driver_add(&st480_init_info) < 0)
		printk(KERN_ERR "mag_driver_add failed!\n");

	return 0;
	printk("st480_init  end okay i2c_number=%d\n",hw->i2c_num); 
