...
static void sensor_init(void)
{
	...
	//write_cmos_sensor_16bit(0x31B4, 0x5392);  // MIPI_TIMING_0	
	write_cmos_sensor_16bit(0x31B6, 0x242B);  // MIPI_TIMING_1
	//write_cmos_sensor_16bit(0x31B6, 0x542B);  // MIPI_TIMING_1
	...
	//write_cmos_sensor_16bit(0x31BC, 0x068B);  // MIPI_TIMING_4	
	...
	//write_cmos_sensor_16bit(0x32C8, 0x0854);  // PDAF_SEQ_START
	//write_cmos_sensor_16bit(0x32CA, 0x0486);  // PDAF_ODP_LLENGTH
	write_cmos_sensor_16bit(0x0342, 0x12B8);  // LINE_LENGTH_PCK
	write_cmos_sensor_16bit(0x0340, 0x0C42);  // FRAME_LENGTH_LINES
	write_cmos_sensor_16bit(0x0202, 0x01FF);  // COARSE_INTEGRATION_TIME
#ifdef PE_16
...
//[PE test]
	...
	//write_cmos_sensor_16bit(0x3780, 0x8000);  //if use PDAF,0x3780 must write!!   // poly_sc_enable //Corn
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("ar1337:i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("ar1337:Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
