...
/*----------------------------------------------------------------------------*/
static int ap3xx6_enable_ps_calibration(struct i2c_client *client, int enable)
{
	struct ap3xx6_priv *obj = i2c_get_clientdata(client);
	u8 databuf[2];
	int res = 0;
...
#if 1
/*******************************************/
	ap3xx6_set_PSTtime(client, 0x01);
...
static int ap3xx6_Calibration_every_time(struct i2c_client *client)
{
	...
	ap3xx6_enable_ps_calibration(client, 1);
	...
	if((flag == 0)){
		if(ui_ps_data > 400)//厚屏
		{
			ui_ps_data = ui_ps_data - 100;
			flag = 1;
			re_val = 0x01;
			//如果set_PSTtime 2 底噪大约为725 > 0x1ff 不能超出0x28 0x29的范围
			//如果set_PSTtime 1 底噪大约为457  0x1D6
			ap3xx6_write_reg(client, 0x25, 0xFF, 0x00, re_val);
		}else{//薄屏
			re_val = 0x06;
			flag = 1;
			ap3xx6_write_reg(client, 0x25, 0xFF, 0x00, re_val);//set_PSTtime 6
		}		
	}
