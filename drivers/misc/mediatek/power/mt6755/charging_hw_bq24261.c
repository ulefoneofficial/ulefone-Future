...
static unsigned int charging_hw_init(void *data)
{
    ...
    //in bq24261_set_ichg
    bq24261_config_interface(bq24261_CON4, 0xF, CON4_ICHRG_MASK,CON4_ICHRG_SHIFT); // ICHG to BAT 500mA + (100+200+400+800)mA =2A
	...
	if(CONFIG_BATTERY_HIGH_VOLTAGE == 4400)
		bq24261_config_interface(bq24261_CON2, 0x2E,0x3F,2); // VBAT_CV
	#else
    bq24261_config_interface(bq24261_CON2, 0x23,0x3F,2); // VBAT_CV
	#endif
    
    bq24261_config_interface(bq24261_CON6, 0x0, CON6_VINDPM_OFF_MASK, CON6_VINDPM_OFF_SHIFT); // VINDPM_OFF 1:10.1V 0:4.2V
    bq24261_config_interface(bq24261_CON5, 0x4, CON5_VINDPM_MASK, CON5_VINDPM_SHIFT); // VINDPM = VINDPM_OFF + (%2 + %4 + 8%)VINDPM_OFF
...
static unsigned int charging_get_current(void *data)
{
    ...
    *(unsigned int *)data = charging_value_to_parameter(CS_VTH,array_size,reg_value);
	// battery_log(BAT_LOG_FULL, "call dump_stack()");
	// dump_stack();
	battery_log(BAT_LOG_FULL, "[charging_get_current]reg_value=0x%x ICharging=%u\n", reg_value,*(unsigned int *)data);
...
static unsigned int charging_set_current(void *data)
{
	...
	battery_log(BAT_LOG_FULL,"[charging_set_current]g_temp_CC_value = %d,set_chr_current = %d,register_value = 0x%x\n",current_value,set_chr_current,register_value);
...
static unsigned int charging_set_hv_threshold(void *data)
{
	...
	battery_log(BAT_LOG_FULL,"[%s:%d]voltage = %d,set_hv_voltage = %d,register_value = 0x%x\n",__FUNCTION__,__LINE__,voltage,set_hv_voltage,register_value);
...
static unsigned int charging_set_ta_current_pattern(void *data)
{
	...
	#ifdef CONFIG_BATTERY_HIGH_VOLTAGE
	if (CONFIG_BATTERY_HIGH_VOLTAGE ==4400)
		cv_voltage = BATTERY_VOLT_04_440000_V;
	#endif

    charging_get_charging_status(&charging_status);
	battery_log(BAT_LOG_FULL,"[%s]%d:increase = %d charging_status = %d\n",__FUNCTION__,__LINE__,increase,charging_status);
    ...
    if(increase == KAL_TRUE)
    {
        ...
        battery_log(BAT_LOG_FULL, "mtk_ta_increase() on short 1");
        msleep(85);
        ...
        bq24261_set_in_limit(0x2); /* 500mA */
        battery_log(BAT_LOG_FULL, "mtk_ta_increase() on short 2");
        msleep(85);
        ...
        bq24261_set_in_limit(0x2); /* 500mA */
        battery_log(BAT_LOG_FULL, "mtk_ta_increase() on long 1");
        msleep(281);
        ...
        bq24261_set_in_limit(0x2); /* 500mA */
        battery_log(BAT_LOG_FULL, "mtk_ta_increase() on long 2");
        msleep(281);
        ...
        bq24261_set_in_limit(0x2); /* 500mA */
        battery_log(BAT_LOG_FULL, "mtk_ta_increase() on long 3");
        msleep(281);
...
static unsigned int charging_set_vbus_ovp_en(void *data)
{
	...
	battery_log(BAT_LOG_FULL,"[%s:%d]e = 0x%x\n",__FUNCTION__,__LINE__,e);
	//dump_stack();
...
static unsigned int charging_set_chrind_ck_pdn(void *data)
{
	...
	battery_log(BAT_LOG_FULL,"[%s:%d]pwr_dn = 0x%x\n",__FUNCTION__,__LINE__,pwr_dn);
...
static unsigned int charging_set_vindpm(void *data)
{
	...
	battery_log(BAT_LOG_FULL,"[%s %d] --------------VinDPM = 0x%x------------\n",__FUNCTION__,__LINE__,v);
	// if(SWITCH_CHR_VINDPM_9V == v)
	// {
		// v = 0x0;//VinDPM = VinDPM_OFF + (1+8%)VinDPM_OFF
		// bq24261_set_vindpm_off(1);//set VinDPM_OFF = 10.1V
	// }else if(SWITCH_CHR_VINDPM_7V == v)
	// {
		
	// }
	// else /*5V*/
	{
		v = 0x4;//VinDPM = VinDPM_OFF + (1+8%)VinDPM_OFF
		bq24261_set_vindpm_off(0);//set VinDPM_OFF = 4.2V
	}

	bq24261_set_vindpm(v);
