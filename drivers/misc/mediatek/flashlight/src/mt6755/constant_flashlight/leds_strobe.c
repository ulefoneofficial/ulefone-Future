...
int LED1Closeflag = 0;
#ifdef SINGLE_FLASH
//上层不再发led2 on off命令 所以LED1 enable和disable函数都不能再去判断LED2Closeflag
int LED2Closeflag = 1;
#else
int LED2Closeflag = 0;
#endif
	...
	else if(LED2Closeflag == 1)
	{
		...
		if(isMovieMode[m_duty1+1][m_duty2+1] == 1)
		{
			...
			*/
			...
			#ifdef SINGLE_FLASH
			//假双闪 上层虽然不发 LED2 On Off的命令 但是driver让LED2灯开和关
			writeReg(SGM3784_REG_ENABLE, g_EnableReg|0x03);	//led1 on
			#else
...
int setDuty_SGM3784_1(int duty)
{
	...
	else if(LED2Closeflag == 1)		//led2 close
	{
		if(isMovieMode[m_duty1+1][m_duty2+1] == 1)
		{
			...
			#ifdef SINGLE_FLASH
			//假双闪 上层虽然不发 LED2 On Off的命令 但是driver让LED2灯开和关
			writeReg(SGM3784_REG_TORCH_LED2,torchLED1Reg[m_duty1+1]);
		...
		else
		{
			...
			writeReg(SGM3784_REG_FLASH_LED2,flashLED1Reg[m_duty1+1]);
			#endif
