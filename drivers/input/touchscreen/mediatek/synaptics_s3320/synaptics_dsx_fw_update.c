...
#define FW_IMAGE_NAME "synaptics/startup_fw_update.img" //for thin tp
#define FW_IMAGE_NAME_TXD "synaptics/startup_fw_update_txd.img" //for thickness tp
#define FW_CONFIG_THIN 0x40	//for thin tp
#define FW_CONFIG_THICKNESS 0x50 //for thickness tp
...
static int fwu_start_reflash(void)
{
	...
	/* Get device config ID */
		retval = fwu->fn_ptr->read(fwu->rmi4_data,
					...
					config_id,
					sizeof(config_id));
					printk("qqqqqqqqqqqqqq config_id[0] = %x\n",config_id[0]);
			if(config_id[0] == FW_CONFIG_THIN )//for thin tp 
			{
				strncpy(fwu->image_name, FW_IMAGE_NAME, MAX_IMAGE_NAME_LEN);
			}
			else if(config_id[0] == FW_CONFIG_THICKNESS )//for thickness tp
			{
...
{
	int retval;
	printk("s2714 updata fwu 5555555\n");
...
static ssize_t tp_fw_status_show(struct device *dev,struct device_attribute *attr, char *buf){	
 		int retval;
	unsigned char config_id[4];
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!tp_kobj) {	
		printk("s2714 error 111111111111\n");
		goto error_sysfs;	
	}	
	
	retval = sysfs_create_group(tp_kobj,&tp_attribute_group);	
	if (retval < 0)	{		
...
{
	printk("s2714 updata fwu 111111111\n");
