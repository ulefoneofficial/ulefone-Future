...
int __batt_meter_init_cust_data_from_dt(struct platform_device *dev)
{
	...
	printk("deng, __batt_meter_init_cust_data_from_dt\n");
	...
	if (!of_property_read_u32(np, "charge_full_design", &val)) {  //add by dengwenhao
		dt_charge_full_design = (int)val;
		bm_debug("Get dt_charge_full_design: %d\n",
			 dt_charge_full_design);
	} else {
		bm_err("Get dt_charge_full_design failed\n");
	}

	if (!of_property_read_u32(np, "charge_full", &val)) {  //add by dengwenhao
		dt_charge_full = (int)val;
		bm_debug("Get dt_charge_full: %d\n",
			 dt_charge_full);
	} else {
		bm_err("Get dt_charge_full failed\n");
	}
